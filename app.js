...
registerForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (userCred.user) {
      // Create user document in Firestore with social features
      ...
      await usersRef.doc(userCred.user.uid).collection('items').add({
...
async function followUser(targetUserId) {
  ...
  try {
    await db.runTransaction(async (transaction) => {
      ...
      const followRef = followsRef.doc(`${currentUser.uid}_${targetUserId}`);
      
      // Update follower counts
      const followerRef = usersRef.doc(targetUserId);
      const followingRef = usersRef.doc(currentUser.uid);
      ...
      transaction.update(followerRef, {
        'stats.followers': firebase.firestore.FieldValue.increment(1)
      });
      
      transaction.update(followingRef, {
        'stats.following': firebase.firestore.FieldValue.increment(1)
      });
    });
  } catch (e) {
    showError(document.body, 'Error following user. Please try again.');
  }
}
...
async function unfollowUser(targetUserId) {
  ...
  try {
    ...
    showError(document.body, 'Error unfollowing user. Please try again.');
  }
}

// Create a post for completed item
async function createCompletionPost(itemId, caption, photoUrl = null) {
  if (!currentUser) return;
  
  try {
    const itemRef = usersRef.doc(currentUser.uid).collection('items').doc(itemId);
    const itemDoc = await itemRef.get();
    const itemData = itemDoc.data();
    
    if (!itemData.checked) {
      throw new Error('Can only create posts for completed items');
    }
    ...
    await db.runTransaction(async (transaction) => {
      ...
      transaction.update(userRef, {
        'stats.totalPosts': firebase.firestore.FieldValue.increment(1)
      });
    });
    
  } catch (e) {
    showError(document.body, 'Error creating post. Please try again.');
    throw e;
  }
}
...
async function toggleLike(postId) {
  ...
  const likeRef = likesRef.doc(`${currentUser.uid}_${postId}`);
  ...
  try {
    ...
    showError(document.body, 'Error updating like. Please try again.');
  }
}
